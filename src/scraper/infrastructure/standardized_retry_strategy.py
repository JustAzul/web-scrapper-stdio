"""
StandardizedRetryStrategy - Async-only retry strategy pattern

This is the standardized version of RetryStrategyPattern that eliminates
...
class StandardizedRetryStrategy:
    """
    Standardized retry strategy that uses only async patterns.

    This class eliminates the async/sync inconsistency found in the original
    RetryStrategyPattern by providing only async methods and adapting sync
    ...
    def __init__(self, max_retries: int, initial_delay: float = 1.0):
        """
        Initialize standardized retry strategy.
    ...
    async def execute(self, async_func: Callable[..., T], *args, **kwargs) -> T:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                self.logger.warning(
                    f"Async attempt {attempt + 1} failed: {e}. Retrying in {delay:.2f}s..."
                )
    ...
    async def execute_sync_as_async(
        ...
        """
        Execute sync function as async using thread pool, with retry and exponential backoff.
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                delay = self.initial_delay * (2**attempt)
                self.logger.warning(
                    f"Sync-as-async attempt {attempt + 1} failed: {e}. Retrying in {delay:.2f}s..."
                )
                await asyncio.sleep(delay)  # Always use async sleep
    ...
    def __repr__(self) -> str:
        """String representation of the standardized retry strategy."""
        return (
            f"StandardizedRetryStrategy(max_retries={self.max_retries}, "
