...
async def _navigate_and_handle_errors(page, url, timeout_seconds):
    try:
        ...
        if is_likely_404:
            ...
            return response, f"[ERROR] HTTP status code: {status_code} or page indicates 'Not Found'"
        ...
        if "net::ERR_NAME_NOT_RESOLVED" in str(
                e) or "net::ERR_CONNECTION_REFUSED" in str(e):
...
def _handle_cloudflare_block(html_content, page_url):
    if _detect_cloudflare_challenge(html_content):
        ...
        return True, "Cloudflare challenge or anti-bot screen detected. Content extraction blocked."
    return False, None
