...
class RefactoredWebScrapingService:
    ...
    async def scrape_url(self, request: ScrapingRequest) -> Dict[str, Any]:
        """
        Orchestrate the complete web scraping workflow using specialized handlers.

        This method now takes a single ScrapingRequest parameter instead of 10 individual
        parameters, following the Parameter Object pattern.

        Args:
            request: ScrapingRequest containing all scraping parameters

        Returns:
            Dictionary with title, final_url, content and error message
        """
        ...
        try:
            ...
            timeout_seconds = request.get_effective_timeout() or 30  # Default timeout
