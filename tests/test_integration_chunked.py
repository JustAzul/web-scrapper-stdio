...
class TestChunkedProcessingIntegration:
    ...
    def test_automatic_processing_method_selection(self, small_website_html, large_website_html):
    ...
    def test_no_breaking_changes_with_edge_cases(self):
        ...
        html_whitespace = "<html><head><title>Whitespace</title></head><body>   \n\t   </body></html>"
        title, clean_html, text_content, error, soup = extract_clean_html(
    ...
    def test_processor_configuration_integration(self):
        ...
        assert len([line for line in text_content.split('\n') if 'Test content' in line]) == 1000


class TestChunkedProcessingMemoryEfficiency:
    """Tests specifically for memory efficiency improvements."""
    
    def test_memory_usage_with_large_content(self):
        """Test that chunked processing uses memory more efficiently."""
        # Generate very large HTML content
        large_content = "<html><head><title>Memory Test</title></head><body>"
        for i in range(1000):
            large_content += f"<div class='block-{i}'>{'Content block ' * 50}</div>"
        large_content += "</body></html>"
        
        elements_to_remove = ['script', 'style']
        url = "http://memory-test.com"
        
        # Process with chunked processor
        processor = ChunkedHTMLProcessor()
        title, clean_html, text_content, error, soup = processor.extract_content(
            large_content, elements_to_remove, url
        )
        
        # Should successfully process large content
        assert error is None
        assert title == "Memory Test"
        assert "Content block" in text_content
        
        # Should have memory metrics
        metrics = processor.get_last_processing_metrics()
        assert metrics['used_chunked_processing'] is True
        assert metrics['content_size_mb'] > 0.5  # Should be substantial content
    
    def test_graceful_degradation_on_memory_pressure(self):
        """Test that system gracefully handles memory pressure."""
        # Reduced content size for testing
        huge_content = "<html><head><title>Huge Content</title></head><body>"
        
        # Generate large but manageable content
        for i in range(500):  # Reduced from 5000 to 500
            huge_content += f"""
            <article>
                <h2>Article {i}</h2>
                <div>{'Very long content that simulates large pages. ' * 20}</div>  # Reduced from 100 to 20
            </article>
            """
        huge_content += "</body></html>"
        
        elements_to_remove = ['script', 'style']
        url = "http://huge-content.com"
        
        # Should handle huge content without crashing
        try:
            processor = ChunkedHTMLProcessor(
                chunk_size_threshold=1024 * 50,  # 50KB chunks
                memory_limit_mb=50  # Limit memory to 50MB
            )
            title, clean_html, text_content, error, soup = processor.extract_content(
                huge_content, elements_to_remove, url
            )
            
            # If successful, verify basic functionality
            if error is None:
                assert title == "Huge Content"
                assert "Article 0" in text_content
            else:
                # If there's an error, it should be informative, not a crash
                assert isinstance(error, str)
                assert len(error) > 0
            
        except MemoryError:
            # If memory error occurs, it should be caught and handled
            pytest.skip("System under memory pressure - expected behavior")
        except Exception as e:
            # Should not have other unexpected exceptions
